...
for subdir, dirs, files in os.walk(srchstr):
    for file in files:
        ...
        if  filepath.endswith(".wav")  and (("Dark" in str(filepath)) or ("Noir" in str(filepath)))  and ("ZZ Indie" not in str(filepath)): 
        ...
        if  filepath.endswith(".wav")  and (("Beat" in str(filepath)) or ("Drum" in str(filepath)))  and ("Ind" in str(filepath)) : 

            contentbeats.append(filepath)

        if  filepath.endswith(".wav")  and ("Bass" in str(filepath)) and ("Step"  not in str(filepath)) : 
