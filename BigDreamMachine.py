...
for subdir, dirs, files in os.walk(srchstr):
    for file in files:
        ...
        if  (sz > 80000000) and filepath.endswith(".wav") and ("Track" in str(filepath)) and ("With" not in str(filepath)) and ("Generated" not in str(filepath)) and ("Oriondrive" not in str(filepath))and ("Futurelight" not in str(filepath)) and ("FutureLight" not in str(filepath)) and (modtim > 1219000000):
